...
@objc public class Constants: NSObject {
    ...
    var permissions: [SPRequestPermissionType] =  [.locationWhenInUse, .notification]
    ...
    func permissionsCheck(vc: UIViewController) {
        if !SPPermissionsManager.init().isAuthorizedPermission(.locationWhenInUse) || !SPPermissionsManager.init().isAuthorizedPermission(.notification) {
            SPRequestPermission.dialog.interactive.present(on: vc, with: permissions, dataSource: CustomDataSource())
        }
    }
